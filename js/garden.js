...
// FIX (1.3): Fallback now uses spread-out values instead of passing 7 args to rgba()
Garden.randomrgba = function (rmin, rmax, gmin, gmax, bmin, bmax, a) {
  ...
  if (Math.abs(r - g) <= limit && Math.abs(g - b) <= limit && Math.abs(b - r) <= limit) {
    // Colors too similar (grey) — use spread-out values to ensure visible blooms
    return Garden.rgba(rmin, gmax, bmax, a);
  } else {
